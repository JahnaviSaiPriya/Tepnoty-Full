...
exports.login = async (req, res) => {
    ...
    if (user && await bcrypt.compare(password, user.password)) {
        const token = jwt.sign({ user_id: user.user_id }, process.env.JWT_SECRET , {expiresIn : '40m'});
        console.log(token)
        await user.save()
...
//     const newToken = jwt.sign({ user_id: decoded.user_id }, process.env.JWT_SECRET);
//     res.send(newToken)
// }
...
exports.refresh = async (req, res) => {
    try {
        ...
        console.log(deviceId);
        const jtoken = token.split(' ')[1];
        console.log("JWT Token: ", jtoken);

        const decoded = jwt.verify(jtoken, process.env.JWT_SECRET);
        const device = decoded.deviceVerification
        if(deviceId == device){
            const newToken = jwt.sign({ user_id: decoded.user_id }, process.env.JWT_SECRET, { expiresIn: '40m' });
        ...
        res.status(500).send({ error: 'Failed to refresh token', message: error.message });
    }
};
